 ...
 */
private[spark]
final class MysortShuffleBlockFetcherIterator(
    context: TaskContext,
  ...
  import MysortShuffleBlockFetcherIterator._
  ...
  private[mysort] def releaseCurrentResultBuffer(): Unit = {
  ...
  private[this] def sendRequest(req: FetchRequest) {
    ...
    val blockFetchingListener = new BlockFetchingListener {
      override def onBlockFetchSuccess(blockId: String, buf: ManagedBuffer): Unit = {
        ...
        MysortShuffleBlockFetcherIterator.this.synchronized {
          if (!isZombie) {
    ...
    for ((address, blockInfos) <- blocksByAddress) {
      ...
      if (address.executorId == blockManager.blockManagerId.executorId) {
        ...
        while (iterator.hasNext) {
          ...
          // Skip empty blocks
          if (size > 0) {
            // logInfo("remote block " + blockId.toString + " " + size + " bytes")
...
private class BufferReleasingInputStream(
    ...
    private val iterator: MysortShuffleBlockFetcherIterator)
  extends InputStream {
...
private[mysort]
object MysortShuffleBlockFetcherIterator {
   ...
   */
  private[mysort] sealed trait FetchResult {
    val blockId: BlockId
   ...
   */
  private[mysort] case class SuccessFetchResult(
      blockId: BlockId,
   ...
   */
  private[mysort] case class FailureFetchResult(
      blockId: BlockId,
      address: BlockManagerId,
      e: Throwable)
    extends FetchResult
