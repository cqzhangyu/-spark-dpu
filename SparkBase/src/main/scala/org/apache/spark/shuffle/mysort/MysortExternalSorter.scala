 ...
 */
private[spark] class MysortExternalSorter[K, V, C](
    context: TaskContext,
  ...
  def insertAll(records: Iterator[Product2[K, V]]): Unit = {
    ...
    val shouldCombine = false
    // logInfo("ExternalSorter.insertAll")
  ...
  def writePartitionedFile(
    ...
    if (spills.isEmpty) {
      ...
      while (it.hasNext) {
        ...
        // logInfo("partitionId" + partitionId + "lengths(partitionId) " + segment.length)
      }
    } else {
      ...
      for ((id, elements) <- this.partitionedIterator) {
        if (elements.hasNext) {
          ...
          // logInfo("lengths(id) " + segment.length)
        }
      }
    }
